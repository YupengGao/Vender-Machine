	...
	public boolean payment(Double totalPayment) {
		...
		while(flag){
			if(cur<totalPayment){
				...
				switch(type){
				case 1:
					System.out.println("Please select coin domination: 1--quarter, 2--50 cents, 3--1 dollar, 4--Cancel");
					select=sc.nextInt();
					if(select==1){
						cur+=0.25;
					}else if(select==2){
					...
					break;
				case 2:
					System.out.println("Please select cash domination: 1--1 dollar, 2--5 dollar, 3--10 dollar, 4--20 dollar, 5--Cancel");
					select=sc.nextInt();
					if(select==1){
						cur+=1;
					}else if(select==2){
